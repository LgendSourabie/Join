...
function forlooprender(test) {
  ...
  container.innerHTML = '';
  if (todo.length === 0) {
    container.innerHTML = `<img class="notask" src="../assets/notasktodo.svg" alt="Empty">`;
  } else {
...
function renderPrio(prio, id) {
  if (tasks[id]['prio'] == 'low') {
    document.getElementById(prio).src = '../assets/priolow.svg';
  } else if (tasks[id]['prio'] == 'medium') {
    document.getElementById(prio).src = '../assets/priomedium.svg';
  } else {
    document.getElementById(prio).src = '../assets/priourgent.svg';
  }
}
...
function createSubtasks(i) {
  ...
  for (let j = 0; j < tasks[i].subtasks.length; j++) {
    ...
    subtasksContainer.innerHTML += /*html*/ `
      ...
      tasks[i].checkboxStates[j] ? '../assets/checkButton.svg' : '../assets/uncheckBox.svg'
